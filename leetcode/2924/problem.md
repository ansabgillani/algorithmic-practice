...
You are given the integer `n` and a **0-indexed** 2D integer array `edges` of length `<font face="monospace">m</font>` representing the **DAG**, where `edges[i] = [u<sub>i</sub>, v<sub>i</sub>]` indicates that there is a directed edge from team `u<sub>i</sub>` to team `v<sub>i</sub>` in the graph.
